...
public class FaceAPIServiceImpl implements FaceAPIService {
  ...
  public HttpResponse<String> faceDetect(byte[] blobContent) throws IOException, InterruptedException {
    HttpRequest httpRequest = buildBaseFaceAPIHttpRequest(FACE_API_BASE_ENDPOINT + "/face/v1.0/detect").header(CONTENT_TYPE_HEADER,
      APPLICATION_OCTET_STREAM).POST(BodyPublishers.ofByteArray(blobContent)).build();
