...
public class DetectionServiceImpl implements DetectionService {
  ...
  @Override
  public void auditAuthorizedDetection(IdentifyResult identifyResult, String filename) {
    identifyResult.candidates()
      .stream()
      .max(Comparator.comparing(IdentifyCandidate::confidence))
      .ifPresent(candidate -> registerAuthorizedDetection(identifyResult, candidate, filename));
  }
  ...
  public void auditUnauthorizedDetection(String filename, String faceId, Logger logger) {
    ...
    faceAlreadyNotified.ifPresentOrElse(
      face -> logger.log(Level.WARNING, String.format("FaceId %s has been already notified", faceId)), () -> {
  ...
  private void registerAuthorizedDetection(IdentifyResult identifyResult, IdentifyCandidate candidate, String filename) {
    ...
    persistenceServiceAuthorizedMembers.lastDetectionByPersonID(actualDetection.getPersonId())
      .stream()
      .findFirst()
      .ifPresentOrElse(lastDetection -> registerDetectionAvoidingFlood(actualDetection, lastDetection.getDetectionTime()),
        () -> persistenceServiceAuthorizedMembers.createDetection(actualDetection));
  }

  private void registerDetectionAvoidingFlood(AuthorizedDetection actualDetection, LocalDateTime detectionTime) {
